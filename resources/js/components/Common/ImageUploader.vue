    ...
    <div class="mb-4">
        <div class="d-flex justify-content-center">
            <div class="upload__box-only-name w-100">
                <div
                    class="upload__img-wrap-only-name specially-for-name"
                ></div>
                <div
                    :class="[
                ...
                >
                    <div
                        v-if="imageDisplayData.length"
                        class="upload__img-wrap-only-name"
                    >
                        <div
                            v-for="(image, index) in imageDisplayData"
                            :key="index"
                            class="img-container w-112"
                        >
                            <!-- Display blob images -->
                            <img
                                :src="image.url"
                            ...
                            <button
                                type="button"
                                @click.prevent="removeImage(index, image)"
                    ...
                    <label class="material-upload w-100 bg-yellow">
                        <p>
...
const onFilesChange = (event) => {
    ...
    if (props.multiple) {
        ...
        // Merge existing and new images for display
        imageDisplayData.value = [...newImageDisplayData];
    }
...
const uploadImages = async () => {
    ...
    try {
        ...
        if (props.multiple) {
            ...
            uploadedImageIds.value = [...newAttachmentIds];
        }

        emit("handleAttachmentUpload", uploadedImageIds.value);
    } catch (error) {
